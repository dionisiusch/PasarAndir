...
<div class="container-fluid">
    ...
    function single(id,index,receipt_id){
    $.ajax({
    ...
    async: false,
    success: function(response) {
    pembayaran["pembayaran"] = response.payment;
    pembayaran["tgl_pembayaran"] = response.created_at;
    },
    ...
    var detail = pembayaran(receipt_id);
    ...
    var tgl_pembayaran = detail['tgl_pembayaran'];
      ...
      html += "<table class='table table-bordered' style='font-size: 10px;width:200px;height:400px'>\
                        <tbody id='modal-invoice'>\
                            ...
                            <tr>\
                                <td style='font-weight:bold'>TGL PEMBAYARAN</td>\
                                <td colspan='3' id='grace_date'>"+tgl_pembayaran.substring(0,10)+"</td>\
                            </tr>\
                            <tr>\
...
function multiple(){
    $.ajax({
    url:'/showprintlistreceipt',
    type: 'get',
    dataType:'json',
    success: function(response) {
    for(var i=0;i<response.length;i++){
        single(response[i].invoice_id,i+1,response[i].receipt_id);
    }
